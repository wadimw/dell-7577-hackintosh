 ...
 *
 * Disassembly of SSDT-2-PegSsdt.aml, Mon Oct  7 00:17:24 2019
 *
 * Original Table Header:
 *     Signature        "SSDT"
 *     Length           0x00002A02 (10754)
 *     Revision         0x02
 *     Checksum         0xEE
 *     OEM ID           "PegSsd"
...
DefinitionBlock ("", "SSDT", 2, "PegSsd", "PegSsdt", 0x00001000)
{
    ...
    External (TMP0, UnknownObj)    // (from opcode)
    External (TMP1, UnknownObj)    // (from opcode)
    External (TMP2, UnknownObj)    // (from opcode)
    ...
    {
        Name (WKEN, Zero)
        OperationRegion (ASPR, PCI_Config, 0xB0, 0x30)
        Field (ASPR, DWordAcc, NoLock, Preserve)
        {
            ASP2,   2
        }
        ...
        OperationRegion (S0F1, SystemMemory, Add (EBAS, 0x1000), 0xF0)
        ...
        PowerResource (PG00, 0x00, 0x0000)
        {
            ...
            Method (_ON, 0, Serialized)  // _ON_: Power On
            {
                ...
                ElseIf (LGreater (OSYS, 0x07D9))
                {
                    ...
                    {
                        Store (0x07FA1028, NVID)
                    }
                    ElseIf (LEqual (Local0, 0x02))
                    {
                        Store (0x07FB1028, NVID)
                    }
                    ElseIf (LEqual (Local0, 0x03))
                    {
                        Store (0x08021028, NVID)
                    }
                    ElseIf (LEqual (Local0, 0x04))
                    {
                        Store (0x08031028, NVID)
                    }

                    If (LEqual (DEID, 0x1C20))
                    {
                        Store (0x40000001, NSOR)
                        If (LEqual (\_SB.GGIV (0x0102000E), One))
                        {
                            Store (One, NHDA)
                        }
                        Else
                    ...
                    ElseIf (LOr (LEqual (DEID, 0x1C8C), LEqual (DEID, 0x1C8D)))
                    {
                        Store (Zero, NHDA)
                    }
