 ...
 *
 * Disassembly of SSDT-3-xh_OEMBD.aml, Sat Mar 16 21:45:36 2019
 *
 * Original Table Header:
 *     Signature        "SSDT"
 *     Length           0x00000967 (2407)
 *     Revision         0x02
 *     Checksum         0xF0
 *     OEM ID           "INTEL"
...
DefinitionBlock ("", "SSDT", 2, "INTEL", "xh_OEMBD", 0x00000000)
{
    ...
    Scope (\_SB.PCI0.XHC.RHUB.HS06)
    {
        Method (_UPC, 0, NotSerialized)  // _UPC: USB Port Capabilities
        {
            If (LOr (LEqual (IPFI, 0x02), LEqual (IPFI, 0x04)))
            {
                Return (GUPC (One))
            }
            Else
    ...
    Scope (\_SB.PCI0.XHC.RHUB.HS08)
    {
        Method (_UPC, 0, NotSerialized)  // _UPC: USB Port Capabilities
        {
            If (LOr (LEqual (IPFI, One), LEqual (IPFI, 0x03)))
            {
                Return (GUPC (One))
            }
            Else
